...
class Backend(object):
    ...
    def getVersion():
        versions = (
            [kb.dbmsVersion]
            if isinstance(kb.dbmsVersion, six.string_types)
            else filterNone(flattenValue(kb.dbmsVersion))
        )
        return None if isNoneValue(versions) else versions[0]
    ...
    def getVersionList():
        ...
        return None if isNoneValue(versions) else versions
